...
from jaxrl5.agents import TD3LagLearner
...
flags.DEFINE_string(
    "project_name", "jaxrl5_quad2d_td3_lag_baseline", "wandb project name."
)
...
flags.DEFINE_string(
    "out_dir",
    "results/QuadrotorTracking2D-v0/jaxrl5_quad2d_td3_lag_baseline",
    "Root directory for experiment outputs.",
)
flags.DEFINE_float("cost_limit", 0.0, "Lagrangian cost limit.")
flags.DEFINE_float("lambda_lr", 1e-3, "Learning rate for Lagrange multiplier.")
flags.DEFINE_float("lambda_init", 0.0, "Initial Lagrange multiplier value.")
flags.DEFINE_float("lambda_max", 1000.0, "Maximum value for Lagrange multiplier.")
...
def _make_eval_policy(agent: TD3LagLearner) -> Callable[[np.ndarray], np.ndarray]:
    eval_agent = agent
...
def main(_):
    ...
    agent: TD3LagLearner = TD3LagLearner.create(
        FLAGS.seed,
        train_env.observation_space,
        train_env.action_space,
        cost_limit=FLAGS.cost_limit,
        lambda_lr=FLAGS.lambda_lr,
        lambda_init=FLAGS.lambda_init,
    ...
    def _save_checkpoint(agent: TD3LagLearner, step: int, observation=None) -> None:
        ckpt_path = ckpt_root / f"ckpt_{step}.msgpack"
        ckpt_path.parent.mkdir(parents=True, exist_ok=True)
        with open(ckpt_path, "wb") as f:
            f.write(serialization.to_bytes(agent))
        try:
            loaded = TD3LagLearner.load(str(ckpt_path))
            assert isinstance(loaded, TD3LagLearner)
    ...
    for step in tqdm.tqdm(
        ...
        if step % FLAGS.eval_interval == 0:
            ...
            metrics_all["train/lambda_value"] = float(agent.lagrangian_lambda)
            metrics_all["eval/episode_cost_limit"] = float(FLAGS.cost_limit)
            ...
            if FLAGS.wandb:
                ...
                print(
                    ...
                    f"lambda={metrics_all['train/lambda_value']:.3f} "
            ...
            append_history(
                ...
                FLAGS.seed,
                {
                    ...
                    "train/lambda_value": metrics_all["train/lambda_value"],
                    "eval/episode_cost_limit": metrics_all["eval/episode_cost_limit"],
                },
            )

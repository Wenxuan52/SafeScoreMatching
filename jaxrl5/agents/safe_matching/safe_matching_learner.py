...
class SafeScoreMatchingLearner(Agent):
    ...
    def _parse_guidance_mode(self, guidance_mode: Optional[str]):
        mode = guidance_mode or "both"
        reward_enabled = mode in ("reward_only", "both")
        safety_enabled = mode in ("safety_only", "both")
        use_constant_lambda = mode == "safety_only"
        return reward_enabled, safety_enabled, use_constant_lambda
    ...
    def _guided_ddpm_sample(
        ...
        reward_enabled, safety_enabled, use_constant_lambda = self._parse_guidance_mode(
            guidance_mode
        )
        skip_guidance = (guidance_mode == "none") or (
            not reward_enabled and not safety_enabled
        )
